...
module.exports = class ElasticApm {
  ...
  publishSourceMap = async ({ asset, map, distDir }) => {
    ...
    const bundleFilePath = `${this.publicPath}/${asset}`;
    ...
    this.logger?.debug(
      `Uploading ${map} to Elastic APM with bundle_filepath: ${bundleFilePath}.`
    );
    ...
    return fetch(url, {
      ...
      .then(([ok, responseText]) => {
        if (ok) {
          this.logger?.debug(`uploaded ${map}.`);
        } else {
          this.logger?.error(`APM server response: ${responseText}`);
          throw new Error(`error while uploading ${map} to Elastic APM`);
        }
      });
  };
  ...
  publishSourceMaps = async ({ sourceMapPattern = '**/*.map', distDir }) => {
    ...
    this.logger?.info(`Elastic APM Beginning Upload`);
